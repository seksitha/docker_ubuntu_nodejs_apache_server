      ...
      return {
        ...
        link: function (scope, elem, attrs) {
          ...
          function createChart (type) {
            if (isResponsive(type, scope) && elem[0].clientHeight === 0 && container.clientHeight === 0) {
              return $timeout(function () {
                createChart(type); // call createChart
              }, 50, false);
            }
            if (! scope.data || ! scope.data.length) return;
            scope.getColour = typeof scope.getColour === 'function' ? scope.getColour : getRandomColour;
            var colours = getColours(type, scope); //call getColour
            var cvs = elem[0], ctx = cvs.getContext('2d'); 
            var data = Array.isArray(scope.data[0]) ?
              getDataSets(scope.labels, scope.data, scope.series || [], colours) : //call getDataSets
              getData(scope.labels, scope.data, colours); // call getData
            var options = angular.extend({}, ChartJs.getOptions(type), scope.options); 

            // Destroy old chart if it exists to avoid ghost charts issue
            // https://github.com/jtblin/angular-chart.js/issues/187
            destroyChart(chart, scope); // call destroyChart
          ...
          scope.$watch('data', function (newVal, oldVal) {
            ...
            if (chart && canUpdateChart(newVal, oldVal)) return updateChart(chart, newVal, scope, elem);

            createChart(chartType);
          }, false);

          //scope.$watch('series', resetChart, true);
          //scope.$watch('labels', resetChart, false);
