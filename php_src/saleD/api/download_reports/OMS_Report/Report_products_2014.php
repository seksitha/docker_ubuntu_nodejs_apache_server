...
$query = "SELECT oms_sale.`id`, inv_refer, dayofmonth(date_bill) as Day, Monthname(date_bill) as Month, Year(date_bill) as Year, date_bill as `Date`,
...
from oms_sale left join `oms_sale_detail` on oms_sale.`id` = `oms_sale_detail`.`main_id` left join info_product
on `oms_sale_detail`.`product` = `info_product`.id Where Year(date_bill) = '2014'";
